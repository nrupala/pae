...
The server never sees plaintext financial data. Encryption and decryption happen exclusively in the client (browser or local application). The server stores and transmits only ciphertext.
...
PAE is designed to be portable across deployment targets:

- **Docker**: Primary development and deployment method
- **Linux VM**: Uses APP_ROOT environment variable for path portability
- **OCI/AWS/GCP**: Terraform templates (planned)
- **Browser-only**: Rust compiled to WASM for zero-server analytics (planned)
